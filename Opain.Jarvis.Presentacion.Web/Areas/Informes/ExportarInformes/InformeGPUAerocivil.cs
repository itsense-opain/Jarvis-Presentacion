...
{
    public class InformeGPUAerocivil
    {
        ...
        public int SumarTotalPOSCobro(List<Anexo17> Anexo17)
        {
            int TotalPOS = 0;
            int TryParsePOS = 0;
            ...
            try
            {
                if (Anexo17.Count > 0  )
                {
                        ...
                        foreach (var item in Anexo17)
                        {
                            ValidarTryParsePOS = Int32.TryParse(item.TotalCOP, out TryParsePOS);
        ...
        public int SumarTotalCobro(List<Anexo17> Anexo17, string TipoCobro)
        {
            int TotalCobro = 0;
            int TryParseCobro = 0;
            ...
            try
            {
                ...
                if (Anexo17.Count > 0 && !string.IsNullOrEmpty(TipoCobro))
                {
                    if (TipoCobro.Equals("COP"))
                        foreach (var item in Anexo17)
                        {
                            ValidarTryParseCobro = Int32.TryParse(item.TarifaCOP, out TryParseCobro);
                        ...
                        foreach (var item in Anexo17)
                        {
                            ValidarTryParseCobro = Int32.TryParse(item.TarifaUSD, out TryParseCobro);
                            if (ValidarTryParseCobro)
                                TotalCobro = TotalCobro + TryParseCobro;
                        }
                }
        ...
        /// </summary>
        /// <param name="Anexo17"></param>
        /// <returns></returns>
        public int SumarTotalMinutos(List<Anexo17> Anexo17)
        {
            int TotalMinutos = 0;
            try
            {
        ...
        public byte[] ArmarExcel(List<Anexo17> Anexo17, string TipoCobro, string filtro1, string filtro2)
        {
            ...
            try
            {
                ...
                //TotalCobro = SumarTotalCobro(Anexo17, TipoCobro); // tarifa cop o usd
                TotalPosCobro = SumarTotalPOSCobro(Anexo17); // total cop
                TotalMinutos = SumarTotalMinutos(Anexo17);
                using (var workbook = new XLWorkbook())
                {
                    //Generamos la hoja
                    var worksheet = workbook.Worksheets.Add("Anexo17");
                    //Generamos la cabecera
                    #region Diseño encabezado
                    if (TipoCobro.Equals("COP"))
                    {
                        worksheet.Range("A1:O1").Merge().Value = "";
                        worksheet.Range("A1:O1").Style.Fill.BackgroundColor = XLColor.White;
                        worksheet.Range("D2:I2").Merge().Value = "Anexo 17 GPU Aerocivil - Jarvis Informe";
                    ...
                    else if (TipoCobro.Equals("USD"))
                    {
                        ...
                        worksheet.Range("D2:H2").Merge().Value = "Anexo 17 GPU Aerocivil - Jarvis Informe";
