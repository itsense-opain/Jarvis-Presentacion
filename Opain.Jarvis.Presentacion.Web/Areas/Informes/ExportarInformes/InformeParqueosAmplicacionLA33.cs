...
{
    public class InformeParqueosAmplicacionLA33 : ExportarInformes.Base
    {
        ...
        public int SumarTotalPOSCobro(List<Anexo13> Anexo13, string TipoCobro)
        {
            int TotalPOS = 0;
            int TryParsePOS = 0;
            ...
            try
            {
                if (Anexo13.Count > 0 && !string.IsNullOrEmpty(TipoCobro))
                {
                    if (TipoCobro.Equals("COP"))
                        foreach (var item in Anexo13)
                        {
                            ValidarTryParsePOS = Int32.TryParse(item.ValorTotalCOP, out TryParsePOS);
                        ...
                        foreach (var item in Anexo13)
                        {
                            ValidarTryParsePOS = Int32.TryParse(item.ValorTotalUSD, out TryParsePOS);
                            if (ValidarTryParsePOS)
                                TotalPOS = TotalPOS + TryParsePOS;
                        }
                }
        ...
        public Decimal SumarTotalCobro(List<Anexo13> Anexo13, string TipoCobro)
        {
            ...
            try
            {
                ...
                if (Anexo13.Count > 0 && !string.IsNullOrEmpty(TipoCobro))
                {
                    if (TipoCobro.Equals("COP"))
                        foreach (var item in Anexo13)
                        {
                            ValidarTryParseCobro = Decimal.TryParse(item.ValorTotalCOP, out TryParseCobro);
        ...
        public Decimal SumarTotalCantidad(List<Anexo13> Anexo14)
        {
            ...
            try
            {
                if (Anexo14.Count > 0)
                {
                    ...
                    {
                        ValidarTryParseCobro = Decimal.TryParse(item.CantidaddeHoras, out TryParseCantidad);
                        if (ValidarTryParseCobro)
        ...
        public byte[] ArmarExcel(List<Anexo13> Anexo13, string TipoCobro, string filtro1, string filtro2)
        {
            Decimal TotalCobro = 0;
            decimal TotalCantidad = 0;
            ...
            try
            {
                //TotalCobroCOP = SumarTotalCobro(Anexo13, "COP");  // unitario
                TotalCantidad = SumarTotalCantidad(Anexo13);

                using (var workbook = new XLWorkbook())
                {
                    //Generamos la hoja
                    var worksheet = workbook.Worksheets.Add("Anexo13");
                    //Generamos la cabecera
                    #region Cabecera excel
                    worksheet.Range("A1:O1").Merge().Value = "";
                    worksheet.Range("A1:O1").Style.Fill.BackgroundColor = XLColor.White;
                    worksheet.Range("D2:I2").Merge().Value = "ParqueosAmplicacionLA33 - Jarvis Informe";
                    ...
                    string SubTotal = "0";
                    foreach (var datos in Anexo13)
                    {
                        //worksheet.Cell(nRow, ).Value = datos.Factura;
                        //worksheet.Cell(nRow, 1).Value = datos.NumerodeFactura;
                        //worksheet.Cell(nRow, 2).Value = datos.Matricula;
                        //worksheet.Cell(nRow, 3).Value = datos.Aerolinea;
                        //worksheet.Cell(nRow, 4).Value = datos.CodigoVueloLlegada;
                        //worksheet.Cell(nRow, 5).Value = datos.CodigoVueloSalida;
                        //worksheet.Cell(nRow, 6).Value = "'" + datos.FechaLlegadaPosicion;
                        //worksheet.Cell(nRow, 7).Value = "'" + datos.FechaSalidaPosicion;
                        //worksheet.Cell(nRow, 8).Value =  datos.HoraIngreso;
                        //worksheet.Cell(nRow, 9).Value = datos.HoraSalida;
                        //worksheet.Cell(nRow, 10).Value = datos.CantidaddeHoras;
                        //worksheet.Cell(nRow, 11).Value = datos.CA;
                        //worksheet.Cell(nRow, 12).Value = datos.Posicion;
                        //worksheet.Cell(nRow, 13).Value = datos.TipoConexion;
                        //worksheet.Cell(nRow, 14).Value = datos.ValorUnitarioCOP;
                        //worksheet.Cell(nRow, 15).Value = datos.ValorTotalCOP;                                             
                        //worksheet.Cell(nRow, 16).Value = datos.ValorUnitariolUSD;
                        //worksheet.Cell(nRow, 17).Value = datos.ValorTotalUSD;
                        worksheet.Cell(nRow, 1).Value = datos.PrefijoFactura;
                        worksheet.Cell(nRow, 2).Value = datos.NumerodeFactura;
                        ...
                        worksheet.Cell(nRow, 12).Value = datos.CantidaddeHoras;
                        ...
                        worksheet.Cell(nRow, 16).Value = datos.ValorUnitarioCOP;
                        worksheet.Cell(nRow, 16).Style.NumberFormat.NumberFormatId = 4;
                        worksheet.Cell(nRow, 17).Value = datos.ValorTotalCOP;
                        worksheet.Cell(nRow, 17).Style.NumberFormat.NumberFormatId = 4;
                        SubTotal = datos.ValorTotalCOP;
                        if (datos.TipoMoneda == "USD")
                        {
                            SubTotal = datos.ValorTotalUSD;
                            worksheet.Cell(nRow, 16).Value = datos.ValorUnitariolUSD;
                            worksheet.Cell(nRow, 17).Value = datos.ValorTotalUSD;
                        }
